(function () {
  ...
  function bindSidebar() {
    ...
    if (!toggle || !backdrop || !body) return;
  ...
  function applyTopOffset() {
    ...
    if (!topNav) return;
    const height = topNav.offsetHeight || 64;
  ...
  function playIncomingMessageTone() {
    ...
    try {
      ...
      window.setTimeout(() => audioCtx.close().catch(() => {}), 260);
    } catch (e) {}
  }
  ...
  function setUnreadPulse(linkId, value) {
    ...
    if (!link) return;
    link.classList.toggle("has-unread", (Number(value) || 0) > 0);
  }
  ...
  function applyUnreadCounts(payload) {
    ...
    if (prevUnreadMessages !== null && messages > prevUnreadMessages) {
      ...
      toast(added > 1 ? `Новых сообщений: ${added}` : "Новое сообщение", "info", 3000);
  ...
  async function fetchUnreadCounts() {
    const hasTargets =
      document.getElementById("navUnreadMessagesCount") ||
  ...
  function normalizeUserIcon() {
    const node = document.querySelector(".nav-user__icon");
    if (!node) return;
